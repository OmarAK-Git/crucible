...
async def run_round_1_adversaries(prompt: str, corpus: str) -> dict:
    """
    ...
    defender_res, challenger_res = await asyncio.gather(
        call_claude_adversary(defender_system, prompt, corpus),
        call_gpt_adversary(challenger_system, prompt, corpus)
    )
